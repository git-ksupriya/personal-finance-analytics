...
def train_rnn(epochs=40, batch_size=64, learning_rate=0.001):
    rnn_dir = os.path.dirname(os.path.abspath(__file__))
    ml_dir = os.path.abspath(os.path.join(rnn_dir, ".."))
    ...
    print("--- Loading Preprocessed Data for 7-Day Multi-Step SimpleRNN ---")
    ...
    model = RNNModel(input_size=1, hidden_size=64, num_layers=2, output_size=7, dropout=0.2)
    criterion = nn.MSELoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)

    print(f"--- Training Multi-Step SimpleRNN Model ({epochs} Epochs) ---")
    model.train()
    ...
    print("\n=== Multi-Step SimpleRNN Model Evaluation Results ===")
    ...
    model_path = os.path.join(rnn_dir, "rnn_model.pt")
    torch.save(model.state_dict(), model_path)
    print(f"\nSimpleRNN model weights saved to {model_path}")

    # 2. Save Metrics JSON
    metrics = {
        "model_name": "Multi-Step 7-Day SimpleRNN Spending Predictor",
        "model_type": "RNN",
        "input_timesteps": 10,
    ...
    metrics_path = os.path.join(rnn_dir, "rnn_metrics.json")
    with open(metrics_path, "w", encoding="utf-8") as f:
        json.dump(metrics, f, indent=4)
    print(f"SimpleRNN metrics saved to {metrics_path}")
    ...
    results_csv_path = os.path.join(rnn_dir, "rnn_results.csv")
    res_df.to_csv(results_csv_path, index=False)
    print(f"SimpleRNN test predictions saved to {results_csv_path}")

    # 4. Generate Report MD
    report_md = f"""# SimpleRNN 7-Day Multi-Step Spending Forecast Report
...
"""
    ...
    report_path = os.path.join(rnn_dir, "rnn_report.md")
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(report_md)
    print(f"SimpleRNN human-readable report saved to {report_path}")
